...
void PvBootstrap::InitVmaAllocationCreateInfo(VmaAllocatorCreateInfo &info) {
      ...
      if (surface_construct != nullptr) {
        ...
        if (table.physicalDevice) {
          ...
          if (device_ret) {
            ...
            InitVmaAllocationCreateInfo(info);
        ...
        WARN("Surface constructor is not set!");
      }
    } else {
...
bool PvBootstrap::isExtensionSupported(const char *ext) {
  auto exts = table.physicalDevice.get_extensions();
  return std::find_if(std::begin(exts), std::end(exts),
